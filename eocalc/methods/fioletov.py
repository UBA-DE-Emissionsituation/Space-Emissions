...
class MultiSourceCalculator(EOEmissionCalculator):
    ...
    def run(self, region=MultiPolygon, period=DateRange, pollutant=Pollutant, 
        ...
        # limit defined
        # obs_to_mem_limit = 10000
        obs_to_mem_limit = 2e9
        if len(df_obs) * len(source_df) > obs_to_mem_limit:
            split_operations = True
            split_level = int(numpy.ceil(len(df_obs) * len(source_df)/obs_to_mem_limit))
            #split_level=4
        else:
        ...
        if split_operations is False:
            ...
            s_t = time.time()
            solution = scipy.sparse.linalg.lsqr(sA, linear_system_B,damp=dampening_factor)
            ...
            if split_level <= 25:
                ...
                for idx_m,mon in enumerate(months):
                    ...
                    print('Calculate linear system, sources',len(source_df),'observations',len(df_obs_mon),'month',mon)
                    # raise
                    linear_system_A_mon = tools.multisource_emission_fit(source_df, df_obs_mon, lon_var, lat_var, plumewidth, decay,multiprocessing=multiprocessing)
                    print('up to creation of A took' '%3.3i:%2.2i' % ( int((time.time() - s_t_mon) / 60), int(numpy.mod((time.time() - s_t_mon), 60))), 'Min:Sec')                

                    # 3.2 Create B, subtract bias
                    # to find bias, we grid all obs and take the lowest 5% of each cell
                    # TODO add higher resolution gridding. Arjos triangle?
                    df_obs_mon['iy'] = (numpy.floor((df_obs_mon[lat_var]-min_lat)/resolution_lat)).values.astype(int)
                    df_obs_mon['ix'] = (numpy.floor((df_obs_mon[lon_var]-min_long)/resolution_lon)).values.astype(int)
                    df_obs_mon['iy_ix']=['%i_%i'%(int(numpy.floor((la-min_lat)/resolution_lat)),int(numpy.floor((lo-min_long)/resolution_lon))) for la,lo in zip(df_obs_mon[lat_var],df_obs_mon[lon_var])]   
                    vcd_mean = df_obs_mon.groupby('iy_ix').mean() 
                    bias_grouped = df_obs_mon.groupby('iy_ix')[[vcd_var,lat_var,lon_var]].quantile(.05) 
                    # maybe count for later?
                    # dfg2c = df_obs_mon.groupby('iy_ix').count() 

                    for idx in range(len(vcd_mean.index.values)):
                        bias_grid[idx_m,int(vcd_mean['iy'][idx]),int(vcd_mean['ix'][idx])] = bias_grouped[vcd_var][idx]
                    bias_per_obs = bias_grid[idx_m,df_obs_mon['iy'],df_obs_mon['ix']]
                    linear_system_B_mon = df_obs_mon[vcd_var].values.copy()
                    linear_system_B_mon = linear_system_B_mon - bias_per_obs
                    print('shapetest',linear_system_A_mon.shape,linear_system_B_mon.shape)
                    # 
                    # Damped least squares  --    solve  (   A    )*x = ( b )
                                                # ( damp*I )     ( 0 )
                    dampening_factor = 0.009 # test value depends on size domain and number of obs
                    ...
                    # TODO add loop for multiple fits with different settings
                    s_t = time.time()
                    solution = scipy.sparse.linalg.lsqr(sA, linear_system_B_mon,damp=dampening_factor)
                    print('Fit took' '%3.3i:%2.2i' % ( int((time.time() - s_t) / 60), int(numpy.mod((time.time() - s_t), 60))), 'Min:Sec')                
                    # test quickplot
                    # conversion  mol  * m-2 * km2 -> kg/yr =* 1/hr * m2/km2 * kg / mol  * days/year * hours/day =  (kg*m2)/(km2*yr*mol)
                    source_conversion = (decay * 1e6) * (binas.xm_NO2) * 365. * 24.
                    ...
                    print('Iteration took' '%3.3i:%2.2i' % ( int((time.time() - s_t_mon) / 60), int(numpy.mod((time.time() - s_t_mon), 60))), 'Min:Sec')                
            else:
        ...
        precalc_adjustment_diurnal = 1.34
        precalc_adjustment_monthly = 1.34
        ...
        #     tmp_return['emissions_coarse'] = emissions_coarse
        #     tmp_return['nx_m'] = nx_m
        #     tmp_return['ny_m'] = ny_m
        #     tmp_return['split_step'] = split_step
        ...
        grid = overlay(tmp_return['gpd'].set_crs(epsg=4326), GeoDataFrame({'geometry': [region]}, crs="EPSG:4326"), how='intersection')
        ...
        total_uncertainty = numpy.full(len(grid),0.4) # TODO inc uncertainties for now set value
