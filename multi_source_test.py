...
for filename, region in regions.items():
    results[filename] = MultiSourceCalculator().run(region, DateRange(
        start='2019-01-01', end='2019-01-31'), Pollutant.NO2, add_region_offset=[2., 2.])
...
# adjustment of emissions for temporal things (monthly and daily emission variation as function lifetime)
# add output obs/obs time/number obs per month / hour
# plot at province level / state level NUTS 0 NUTS 2
...
quicktestplot = overlay(results[filename]['gpd'], GeoDataFrame(
    {'geometry': [region]}, crs="EPSG:4326"), how='intersection')
quicktestplot = quicktestplot.set_crs(epsg=4326)
quicktestplot[f"Total {Pollutant.NO2.name} emissions [kg]"] = quicktestplot['fitted_emissions']
ctx.add_basemap(quicktestplot.to_crs(epsg=3857).plot(f"Total {Pollutant.NO2.name} emissions [kg]", figsize=(
    20, 20), legend=True, legend_kwds={'label': f"Emissions in {filename} [kg]", 'orientation': "horizontal"}, alpha=0.7, edgecolor='k'))
